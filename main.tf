terraform {
  backend "s3" {
    bucket         = "terraform-bucket-0205" #replace with your bucket name
    key            = "terraform/state.tfstate"
    region         = "us-west-2" #replace with the right region
...
resource "aws_instance" "web" {
  ami           = "ami-07b0c09aab6e66ee9" # replace with the Amazon Linux AMI value on your account
